...
public class CapitalizationRule extends TokenFilter{
	...
	public TokenStream getStream() {
		...
		while (stream.hasNext()) {
			...
			if(null!=termText && StringUtil.matchRegex(termText, RegExp.REGEX_ALL_CAPS)){
				continue;
			}
			//check for tokens like biwords 'Stanford University'
			else if(null!=termText && null!=nextToken && StringUtil.matchRegex(termText, RegExp.REGEX_FIRST_CAPS) 
					&& StringUtil.matchRegex(nextTermText, RegExp.REGEX_FIRST_CAPS)){
				token.merge(nextToken);
				stream.next();
				stream.remove();
			}
			
			//If the preceding token DOES NOT ends with ./$/! and the current token contains one or more 
			//capital letter then do not convert it into lowercase
			else if(prevTermText!=null && termText!=null && !StringUtil.matchRegex(prevTermText, RegExp.REGEX_SENT_ENDS) 
