...
public class CapitalizationRule extends TokenFilter {
	...
	public void applyFilter() {
		...
		// Find the previous token by doing two previous operations as next is
		// usually same as current
		Token previousToken = stream.getPrevious();
		if (previousToken != null) {
			previousToken = stream.getPrevious();
		}
		String prevTermText = null;
		if (null != previousToken) {
			prevTermText = previousToken.getTermText();
		}
		// Now move the iterator back to it's original position as previous call
		// would have shifted it back.
		stream.next();
		// get the next token
		...
		else if (null != termText && null != nextToken
