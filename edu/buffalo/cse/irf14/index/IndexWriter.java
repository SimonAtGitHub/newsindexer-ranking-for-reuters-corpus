...
public class IndexWriter {
	...
	public void addDocument(Document d) throws IndexerException {
		...
		try {
			 ...
			 */
			String fileId = StringUtil.convertStrArrToString2(d
					.getField(FieldNames.FILEID));
			...
			for (FieldNames fieldName : FieldNames.values()) {
				...
				if (!fieldName.equals(FieldNames.FILEID)) {
					...
					if (stream != null) {
						createDictionaryAndIndex(fieldName, stream, fileId,
								docId);
					}
				}
			}
	...
	/**
	 * Analyze the tokenstream of each document attributes adn apply chained
	 * filters to it
	 * 
	 * @param fieldName
	 * @param d
	 ...
	 */
	private TokenStream analyzeStream(FieldNames fieldName, Document d)
			throws TokenizerException {
		...
		try {
			...
			if (str != null && !str.isEmpty()) {
				...
				Analyzer analyzer;
				analyzer = fact.getAnalyzerForField(fieldName, stream);
	...
	public void close() throws IndexerException {
		writeToDisk(DocumentDictionary.getInstance().getMap(),
		...
		writeToDisk(CategoryDictionary.getInstance().getMap(),
				CommonConstants.CATEGORY_DICTIONARY_FILENAME);
		writeToDisk(TermIndex.getInstance().getMap(),
				CommonConstants.TERM_INDEX_FILENAME);
		writeToDisk(AuthorIndex.getInstance().getMap(),
				CommonConstants.AUTHOR_INDEX_FILENAME);
		writeToDisk(PlaceIndex.getInstance().getMap(),
				CommonConstants.PLACE_INDEX_FILENAME);
		writeToDisk(CategoryIndex.getInstance().getMap(),
	...
	public void writeToDisk(Object object, String fileName) {
		...
		try {
			fileOut = new FileOutputStream(indexDir + File.separatorChar
					+ fileName);
	 ...
	 * @param docId
	 */
	private void createDictionaryAndIndex(FieldNames fieldName,
			TokenStream stream, String fileId, Integer docId) {
		...
		try {
			while (stream.hasNext()) {
				...
				// enter the term in the dictionary and index if term is not in
				// the dictionary
				// if it is not in the dictionary , it is also not in the
				// postings list
				...
				// if the term is already present in the dictionary
					...
					// increase the total frequency and set it in the posting
					// wrapper
					totalFrequency++;
					...
					if (postings != null) {
						for (Posting postingItem : postings) {
							// if the term is already mapped to a doc in the
							// postings list just increase its frequency
						...
						// if the term is not mapped to the doc in the postings
						// list , create a new posting for the term
